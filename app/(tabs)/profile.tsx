...
export default function ProfileScreen() {
  ...
  const fetchProfileData = async () => {
    try {
      ...
      // count total ads watched by this user
      const { count: totalAdsWatched } = await supabase
        .from('ad_views')
        .select('id', { count: 'exact', head: true })
        .eq('user_id', userId);

      // count total ad revenue generated by this user
      const { data: adRevenueData } = await supabase
        .from('ad_views')
        .select('revenue')
        .eq('user_id', userId);

      const totalAdsRevenue = adRevenueData
      ...
      setProfile({
        ...
        avatar: profileData?.avatar_url || 'https://randomuser.me/api/portraits/men/16.jpg',
        totalAdsWatched: totalAdsWatched ?? 0,
